mk_tble_needs_proj_shrt_test = function(data ){
  ...
  temp_table = reactable(
    ...
    ,columns = combined_named_lists(
      ...
      ,colDef_filter_select(cols = rtrn_cols(temp_data, words = "summary|research_need_id_number", exclude = T), id = id)
      ,colDef_filter_select(cols = rtrn_cols(temp_data, words = "research_need_id_number"), id = id, F)
