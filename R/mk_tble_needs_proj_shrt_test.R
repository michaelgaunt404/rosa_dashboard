mk_tble_needs_proj_shrt_test = function(data ){
  ...
  # data = tar_read(data_needed_pro_ex)

  temp_data = data %>%
    select(research_need_id_number, research_category, summary_of_research_need
           ,source_of_identification, location, data_gap_analysis_score_mg) %>%
    rename_with(~str_remove(.x, "mg"))
  ...
  temp_table = reactable(
    ...
    ,columns = combined_named_lists(
      ...
      ,colDef_colWidth_robust(cols = rtrn_cols(temp_data, "id_number|category"), maxWidth = 90)
      ,colDef_colWidth_robust(cols = rtrn_cols(temp_data, "score|location"), maxWidth = 120)
      ...
      ,colDef_filter_select(cols = rtrn_cols(temp_data, words = "summary", exclude = T), id = id)
