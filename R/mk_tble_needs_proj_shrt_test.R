mk_tble_needs_proj_shrt_test = function(data){
  ...
  temp_data = data %>%
  select(research_need_id_number, research_category
  ...
  temp_table = reactable(
    ...
    ,columns = combined_named_lists(
      ...
      ,colDef_colWidth_robust(cols = rtrn_cols(temp_data, "itle"), minWidth = 250)
      ...
      ,colDef_filter_select(cols = rtrn_cols(temp_data, words = "title", exclude = T), id = id)
    ), wrap = T, elementId = id
  ) %>%
    rctble_format_table()
